...
export function HomeClient({ session, sortedFields, bookedFieldIds }: any) {
    ...
    return (
        <div className="flex flex-col bg-gray-50/50">
            ...
            <div className="container mx-auto -mt-24 relative z-40 px-4 pb-20 space-y-8">
                {/* Filter Bar */}
                <Card className="rounded-3xl shadow-2xl border-0 overflow-hidden bg-white/95 backdrop-blur-md">
                    <CardContent className="p-4 md:p-6">
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                            {/* Search */}
                            <div className="md:col-span-5 relative">
                                <Search className={`absolute ${isRtl ? 'right-4' : 'left-4'} top-1/2 -translate-y-1/2 text-gray-400 h-5 w-5`} />
                                <Input
                                    placeholder={t('searchByName')}
                                    className={`${isRtl ? 'pr-12' : 'pl-12'} h-14 bg-gray-50/50 border-gray-100 rounded-2xl text-lg focus:ring-green-500 focus:border-green-500 transition-all`}
                                    value={search}
                            ...
                            <div className="md:col-span-4 relative group">
                                <MapPin className={`absolute ${isRtl ? 'right-4' : 'left-4'} top-1/2 -translate-y-1/2 text-gray-400 h-5 w-5`} />
                                <select
                                    value={selectedArea}
                                    onChange={(e) => setSelectedArea(e.target.value)}
                                    className={`w-full h-14 bg-gray-50/50 border-gray-100 rounded-2xl ${isRtl ? 'pr-12' : 'pl-12'} text-lg focus:ring-green-500 focus:border-green-500 appearance-none cursor-pointer transition-all hover:bg-white`}
                                >
                            ...
                            <div className="md:col-span-3">
                                <Button
                                    ...
                                    className={`w-full h-14 rounded-2xl flex items-center justify-center gap-2 font-bold transition-all ${isNearestActive
                                        ? 'bg-green-600 text-white shadow-[0_8px_20px_rgba(34,197,94,0.4)]'
                                        : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                                        }`}
                                >
                                    <Navigation2 className={`h-5 w-5 ${isNearestActive ? 'animate-pulse' : ''}`} />
                                    {t('nearest')}
                                </Button>
                            </div>
                        </div>
